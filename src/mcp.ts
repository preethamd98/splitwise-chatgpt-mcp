...
const result = (data: unknown) => ({ content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }], structuredContent: data as Record<string, unknown> });
...
export function createMcpServer(client: SplitwiseClient, grantedScopes = new Set(["splitwise:read", "splitwise:write"])) {
  const server = new McpServer({ name: "splitwise", version: "1.0.0" }, {
    instructions: "Use read tools to resolve IDs before writes. Creating an expense changes the user's Splitwise account; summarize the exact amount, currency, group, payer, and shares before calling it.",
  });

  server.registerTool("list_groups", secured({
    title: "List Splitwise groups", description: "List the current user's Splitwise groups, members, and group balances.",
    inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: true },
  }, readSecurity), async () => { requireScope(grantedScopes, "splitwise:read"); return result(await client.listGroups()); });

  server.registerTool("list_friends", secured({
    title: "List Splitwise friends and balances", description: "List the current user's friends and balances, including balances by group.",
    inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: true },
  }, readSecurity), async () => { requireScope(grantedScopes, "splitwise:read"); return result(await client.listFriends()); });
  ...
  }, readSecurity), async (args) => { requireScope(grantedScopes, "splitwise:read"); return result(await client.listExpenses(args)); });
  ...
  server.registerTool("get_expense", secured({
    ...
    annotations: { readOnlyHint: true, openWorldHint: true },
  }, readSecurity), async ({ expense_id }) => { requireScope(grantedScopes, "splitwise:read"); return result(await client.getExpense(expense_id)); });
  ...
  server.registerTool("create_expense", secured({
    ...
    return result(await client.createExpense(args));
  });
  return server;
}
